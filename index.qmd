...
```{r, echo = TRUE}
gghistogram(log(cor.test$share_global_luc_co2))
```

```{r, echo = TRUE}
gghistogram(log(cor.test$gas_co2_per_capita))
```

```{r, echo = TRUE}
gghistogram(log(cor.test$oil_co2_per_capita))
```

```{r, echo = TRUE}
gghistogram(log(cor.test$gdp_percap))
```

```{r, echo = TRUE}
gghistogram(log(cor.test$coal_co2_per_capita))
```

```{r, echo = TRUE}
gghistogram(log(cor.test$share_global_coal_co2))
```

```{r, echo = TRUE}
gghistogram(cor.test$cumulative_luc_co2)
```

```{r, echo = TRUE}
gghistogram(cor.test$share_global_luc_co2)
```
...
rec_energy <- cor.test %>% recipe(share_global_luc_co2,
gas_co2_per_capita, oil_co2_per_capita, gdp_percap, coal_co2_per_capita, share_global_coal_co2, cumulative_luc_co2, share_global_luc_co2) %>%
  ...
  step_naomit(all_predictors(), all_outcomes()) %>%
    step_filter(
    all_predictors() > (quantile(cor.test$all_predictors(), 0.25) - 1.5 * IQR(cor.test$all_predictors())),
    all_predictors() < (quantile(cor.test$all_predictors(), 0.75) + 1.5 * IQR(cor.test$all_predictors()))
  )
