...
top_emitters_res <- govres |>
  group_by(country) |>
  filter(country %in% c("China", "India", "Russia", "United_States", "Japan"))
...
```{r, echo = TRUE}
ggplot(govres, aes(x = factor(date), y = Rigidity_Public_Health, color = country, group = country)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Per Capita GHG Emissions (Excl. Land Use) by Country (2015–2022)",
...
#we want a pivoted so we can do faceted graph of each country's economic and social indicators
...
Facet_Wrap for governmnent COVID rigidity
```{r, echo = TRUE}
...
2.5: look on the our world in data dataset for graphs about energy usage by countries (link in the email)

2.5 if there is a graph, see if there's a faceted graph for main sectors of energy consumption (if data are available). If so, go on to step 4. If not, go to step 3.

3.  make ANOVA for energy consumption graphs, showing the top energy consuming sectors for the top 5 highest ghg emitting countries and top 5 highest energy consumers to see if there's a significant difference in averages over time
